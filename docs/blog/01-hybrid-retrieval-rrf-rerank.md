...
k=60 是 Cormack 等人 2009 年论文验证过的经验值，至今没发现需要调的必要。RRF 的代价是**丢失分数的绝对语义**（融合分只是排名的调和，说不出"这条到底多相关"）——这个坑下面 Rerank 一节正好接住。
...
为什么不用固定阈值？因为 relevance_score 的真实分布在 0.1~0.9 全区间都有，固定线会随语料漂移失准。为什么明知道会砍掉边缘命中还要砍？因为**下游是证据拒答**：如果检索放进来一条弱相关片段，模型会基于它编一个似是而非的答案——这比"承认库里没有"危害大得多。评测上掉的那条，和线上"宁可拒答不可胡说"是同一套哲学在两个层面的投影。
