...
| | 评测与质量 | 评测集 CRUD、三路线离线跑批（Recall@k/MRR）、未命中明细、质量监控（好评率/拒答/缓存/Recall 趋势）、RAGAS 式生成质量四指标（忠实度/答案相关性/上下文精确率/召回率，LLM-as-judge） |
...
- **大小模型路由**：Ollama/vLLM 等 OpenAI 兼容本地端点一键接入；寒暄/超短请求自动分流到本地小模型（低延迟+数据不出域），多模态/工具调用/思维链保持云端；本地失败自动降级云端，决策埋点 `docmind_llm_route_total{backend, reason}`；成本收益用 `python scripts/cost_report.py` 可复现测算（含 FAQ 扩充分流情景分析）
...
口径：同一套 12 个固定问题按并发 1/4/8 各跑一遍（每遍 12 条请求）——第一遍全冷（12 miss，走完整链路），
第二、三遍同题全部命中缓存（24 hit）。

| 场景 | 优化前 | 优化后 | 说明 |
|---|---|---|---|
| 并发 1（第一遍，纯冷查询） | QPS 2.46 / P50 350ms | QPS 2.84 / P50 308ms | 无缓存收益场景，数字同级——缓存不伤冷启动 |
| 并发 4（第二遍，缓存热） | QPS 2.97 / P95 1390ms | **QPS 741 / P95 6ms** | 重复问题跳过全部上游往返 |
| 并发 8（第三遍，缓存热） | QPS 3.1 / P95 2573ms | **QPS 801 / P95 11ms** | 命中率 67%（12 miss + 24 hit）可复现 |
